...
'he': {
    ...
    'home': {
        ...
        'lead':'שיקול דעת מקצועי ועצמאי לארגונים שזקוקים לשקיפות טובה יותר, סביבת בקרה סדורה ופיקוח פיננסי פרקטי — בישראל ובפעילות בינלאומית.',
        ...
        'faqs':[
            ('מתי נכון להיעזר בגורם חיצוני לביקורת פנימית או לבקרות?','כאשר ההנהלה זקוקה לנקודת מבט עצמאית על סיכונים, תהליכים ובקרות, כאשר תחומי האחריות נעשו מורכבים, או כאשר דרישות רגולציה וממשל מחייבות יותר סדר ותיעוד.'),
...
'he': {
'audit':'עבודת ביקורת עצמאית בסביבות פיננסיות וממשל מורכבות.',
...
<section class="proof-strip"><div class="container proof-grid">{proofs}</div></section>
<section class="section"><div class="container"><div class="section-head"><div><div class="eyebrow">{c['nav']['services']}</div><h2>{h['services_title']}</h2></div><p class="section-intro">{h['services_intro']}</p></div><div class="services-grid">{svc}</div></div></section>
<section class="section alt"><div class="container"><div class="section-head"><div><div class="eyebrow">{'Management priorities' if lang=='en' else 'סדרי עדיפויות ניהוליים'}</div><h2>{h['challenges_title']}</h2></div><p class="section-intro">{'The starting point is not a service catalogue. It is the management issue that needs a clearer answer.' if lang=='en' else 'נקודת הפתיחה אינה קטלוג שירותים, אלא הסוגיה הניהולית שזקוקה לתשובה ברורה יותר.'}</p></div><div class="challenge-grid">{chall}</div></div></section>
...
<section class="section"><div class="container"><div class="section-head"><div><div class="eyebrow">{c['nav']['industries']}</div><h2>{h['industries_title']}</h2></div><p class="section-intro">{'Experience matters most when the operating environment is complex, regulated or changing.' if lang=='en' else 'ניסיון חשוב במיוחד כאשר סביבת הפעילות מורכבת, מפוקחת או משתנה.'}</p></div><div class="industry-grid">{industries}</div></div></section>
 <section class="section alt"><div class="container"><div class="section-head"><div><div class="eyebrow">FAQ</div><h2>{h['faq_title']}</h2></div><a class="text-link" href="{url(lang,'faq')}">{'View all questions' if lang=='en' else 'לכל השאלות'}</a></div><div class="faq-list">{faqs}</div></div></section>
 <section class="section"><div class="container"><div class="cta-band"><div><h2>{h['final_title']}</h2><p>{h['final_text']}</p></div><div class="actions"><a class="btn btn-primary" href="{url(lang,'contact')}">{c['cta']}</a></div></div></div></section>'''
...
def generic_body(lang, slug):
    ...
    if slug=='services':
        ...
        body+=f'<section class="section"><div class="container"><div class="industry-grid">{inds}</div></div></section>'
    elif slug=='about':
        paras = [
        ...
        body+=f'<section class="section"><div class="container content-grid"><article class="content-main"><h2>{"A firm built around professional judgement" if lang=="en" else "משרד המבוסס על שיקול דעת מקצועי"}</h2>{ps}</article><aside><div class="side-card"><h3>{"Leadership" if lang=="en" else "הנהלה"}</h3><p>{h["leadership_text"]}</p><a class="text-link" href="{url(lang,"leadership")}">{h["leader_link"]}</a></div></aside></div></section>'
